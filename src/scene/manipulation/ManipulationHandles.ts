...
export type HandleTarget = { kind: "resize"; axis: ManipulationAxis; side: HandleSide } | { kind: "rotate" };
 ...
 * rotation (see layoutHandles() in engine/manipulation/manipulationMath.ts
 * for where each goes and which dimension it drives).
 *
 ...
 * it's easy to grab; getHandleMeshes() returns those for
...
export class ManipulationHandles {
  ...
  /** The handles' hit meshes, to raycast - empty while nothing is selected. */
  getHandleMeshes(): THREE.Object3D[] {
    return this.group.visible ? [...this.resizeHits, this.ringHit] : [];
  }
  ...
  private sync(): void {
    ...
    layout.resize.forEach((handle, index) => {
      const hit = this.resizeHits[index];
