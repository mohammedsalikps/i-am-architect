...
export interface AssetDefinition {
  ...
  url: string;
  /**
   * Relative to src/assets/thumbnails/ (see ui/assetThumbnails.ts) - a
   * real render of this asset's own .gltf, generated by
   * scripts/generate-thumbnails.html, never a hand-drawn substitute.
   * Optional and deliberately unset on every entry below: the library
   * derives "<id>.png" by convention (the same relative-filename-by-id
   * pattern `url` already uses), so a normal new asset needs no thumbnail
   * bookkeeping here at all. Set this only for the rare asset whose
   * thumbnail genuinely can't follow the convention (e.g. sharing another
   * asset's render). A missing file - convention or override - falls back
   * to the existing hand-drawn icon (ui/assetIcons.ts), never a blank or
   * broken card.
   */
  thumbnailUrl?: string;
