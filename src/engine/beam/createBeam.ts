...
/** Makes new beam ids come after `ids` - the same rule as createWall.ts's reserveWallIds(). */
export function reserveBeamIds(ids: Iterable<string>): void {
  for (const id of ids) {
    const match = /^beam-(\d+)$/.exec(id);
    if (match) {
